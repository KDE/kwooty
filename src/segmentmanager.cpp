...
SegmentManager::SegmentManager(Core *parent) : QObject(parent)
{
    ...
    this->mDownloadModel = parent->getDownloadModel();
    this->mItemParentUpdater = parent->getItemParentUpdater();
...
bool SegmentManager::sendNextIdleSegment(QStandardItem *fileNameItem, ClientManagerConn *currentClientManagerConn, SegmentInfoData segmentInfoData)
{
    ...
    for (int segmentIndex = 0; segmentIndex < segmentList.size(); segmentIndex++) {
        ...
        if (!itemFound &&
            ...
            mItemParentUpdater->getItemDownloadUpdater()->updateItems(fileNameItem->index(), nzbFileData);
...
void SegmentManager::setIdlePauseSegments(QStandardItem *fileNameItem, const UtilityNamespace::ItemStatus targetStatus)
{
    ...
    QStandardItem *nzbStatusItem = this->mDownloadModel->getStateItemFromIndex(fileNameItem->index());
    UtilityNamespace::ItemStatus nzbStatus = this->mDownloadModel->getStatusFromStateItem(nzbStatusItem);
...
void SegmentManager::setIdleDownloadFailSegments(QStandardItem *fileNameItem)
{
    ...
    this->mDownloadModel->updateNzbFileDataToItem(fileNameItem, nzbFileData);
    this->mDownloadModel->updateProgressItem(fileNameItem->index(), PROGRESS_INIT);
...
QStandardItem *SegmentManager::searchItem(const SegmentData &segmentData)
{
    ...
    QStandardItem *nzbItem = this->mDownloadModel->item(firstLevelRowPosition, FILE_NAME_COLUMN);
    ...
    if (nzbItem) {
        ...
        if (fileNameItem) {
            ...
            if (this->mDownloadModel->getUuidStrFromIndex(fileNameItem->index()) == segmentData.getParentUniqueIdentifier().toString()) {
...
QStandardItem *SegmentManager::searchItem(const QVariant &parentIdentifer, const UtilityNamespace::ItemStatus itemStatus)
{
    ...
    if ((this->mDownloadModel->rowCount() > 0) &&
        ...
        while ((row < this->mDownloadModel->rowCount()) && !itemUpdated) {

            //retrieve root item (nzb item) :
            nzbFileModelIndex = this->mDownloadModel->item(row, FILE_NAME_COLUMN)->index();

            QStandardItem *nzbStatusItem = this->mDownloadModel->item(row, STATE_COLUMN);
            ...
            if (isSearchInNzb) {
                ...
                QModelIndexList parentIndexList = this->mDownloadModel->match(parentModelIndex,
                                                  IdentifierRole,
                                                  parentIdentifer,
                                                  1,
                                                  Qt::MatchExactly);
                ...
                if (!parentIndexList.isEmpty()) {
                    ...
                    fileNameItem = this->mDownloadModel->itemFromIndex(parentModelIndex);
...
void SegmentManager::getNextSegmentSlot(ClientManagerConn *currentClientManagerConn)
{
    ...
    while ((row < this->mDownloadModel->rowCount()) && !itemFound) {

        QStandardItem *nzbItem = this->mDownloadModel->item(row, FILE_NAME_COLUMN);
        QStandardItem *nzbStatusItem = this->mDownloadModel->item(row, STATE_COLUMN);
        UtilityNamespace::ItemStatus currentStatus = this->mDownloadModel->getStatusFromStateItem(nzbStatusItem);
        ...
        if (Utility::isReadyToDownload(currentStatus) && !itemFound) {
            ...
            for (int i = 0; i < nzbItem->rowCount(); ++i) {
                ...
                if (!itemFound) {
                    ...
                    UtilityNamespace::ItemStatus currentChildStatus = this->mDownloadModel->getStatusFromStateItem(stateItem);
...
void SegmentManager::updateDecodeSegmentSlot(PostDownloadInfoData decodeInfoData)
{
    ...
    if (fileNameItem) {
        ...
        mItemParentUpdater->getItemPostDownloadUpdater()->addFileTypeInfo(decodeInfoData);

        // update items :
        mItemParentUpdater->getItemPostDownloadUpdater()->updateItems(decodeInfoData);
...
void SegmentManager::updateRepairExtractSegmentSlot(PostDownloadInfoData repairDecompressInfoData)
{
    ...
    if (fileNameItem) {
        ...
        // update items :
        mItemParentUpdater->getItemPostDownloadUpdater()->updateItems(repairDecompressInfoData);
...
void SegmentManager::updatePendingSegmentsToTargetServer(const int &currentServerGroup, const int &nextServerGroup, const PendingSegments pendingSegments)
{
    ...
    for (int i = 0; i < this->mDownloadModel->rowCount(); ++i) {
        ...
        QStandardItem *nzbItem = this->mDownloadModel->getFileNameItemFromRowNumber(i);
        UtilityNamespace::ItemStatus currentStatus = this->mDownloadModel->getStatusDataFromIndex(nzbItem->index()).getStatus();
        ...
        if (Utility::isInDownloadProcess(currentStatus)) {
            ...
            for (int j = 0; j < nzbItem->rowCount(); j++) {
                ...
                UtilityNamespace::ItemStatus childStatus = this->mDownloadModel->getChildStatusFromNzbIndex(nzbItem->index(), j);
                ...
                if (Utility::isInDownloadProcess(childStatus)) {
                    ...
                    if (listUpdated) {
                        // update the nzbFileData of current fileNameItem and its corresponding items;
                        nzbFileData.setSegmentList(segmentList);
                        this->mDownloadModel->updateNzbFileDataToItem(childFileNameItem, nzbFileData);

                        mItemParentUpdater->getItemDownloadUpdater()->updateItems(childFileNameItem->index(), nzbFileData);
                    }
