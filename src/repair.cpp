...
void Repair::launchProcess(const QList<NzbFileData>& nzbFileDataList, const QString& par2BaseName){
    ...
    if (isPar2ProgramFound) {
        ...
        if (Utility::isFolderExists(fileSavePath)) {
            ...
            if (!this->par2FilesOrderedList.isEmpty()) {
                ...
                args.append(fileSavePath + par2BaseName);
...
//                                       processing                                             //
//==============================================================================================//
QString Repair::sortPar2FilesBySize(/*const QString& par2BaseName*/){
    ...
    foreach (NzbFileData nzbFileData, this->nzbFileDataList) {
        ...
        if ( nzbFileData.isPar2File() /*&&
             currentFileName.contains(par2BaseName, Qt::CaseInsensitive)*/ ) {
        ...
        if (this->repairProgressValueOld != repairProgressValue) {
            ...
            foreach (NzbFileData currentNzbFileData, this->nzbFileDataList) {
                ...
                if ((currentNzbFileData.getVerifyProgressionStep() == VerifyDamagedStatus) ||
                    ...
                    emit updateRepairSignal(currentNzbFileData.getUniqueIdentifier(), repairProgressValue, RepairStatus, this->getItemTarget(currentNzbFileData));
...
void Repair::removePar2Files(){
    foreach (NzbFileData nzbFileData, this->nzbFileDataList) {
        ...
        if (nzbFileData.isPar2File()){
            //kDebug() << "Remove Par2 file :" << nzbFileData.getFileSavePath() + nzbFileData.getDecodedFileName();
...
void Repair::repairFinishedSlot(const int exitCode, const QProcess::ExitStatus exitStatus){
    ...
    for (int i = 0; i < this->nzbFileDataList.size(); i++) {
        ...
        if (nzbFileData.isPar2File()) {
            ...
            emit updateRepairSignal(nzbFileData.getUniqueIdentifier(), PROGRESS_COMPLETE, VerifyFinishedStatus, ParentItemTarget);
        ...
        if (repairStatus == Repair::RepairComplete) {
            ...
            foreach (NzbFileData nzbFileData, this->nzbFileDataList) {
                if ( (nzbFileData.getVerifyProgressionStep() == RepairStatus) ||
                    ...
                    emit updateRepairSignal(nzbFileData.getUniqueIdentifier(), PROGRESS_COMPLETE, RepairFinishedStatus, ChildItemTarget);
                }
            }
    ...
    // 2. exit with errors :
        ...
        foreach (NzbFileData nzbFileData, this->nzbFileDataList) {
            if ( (nzbFileData.getVerifyProgressionStep() == VerifyMissingStatus) ||
                ...
                emit updateRepairSignal(nzbFileData.getUniqueIdentifier(), PROGRESS_COMPLETE, errorStatus, ChildItemTarget);
            }
        }
...
void Repair::sendPar2ProgramNotFoundNotification() {
    ...
    foreach (NzbFileData nzbFileData, this->nzbFileDataList) {
        ...
        if (nzbFileData.isPar2File() ) {
            ...
            emit updateRepairSignal(nzbFileData.getUniqueIdentifier(), PROGRESS_COMPLETE, Par2ProgramMissing, ParentItemTarget);
        }
    }
...
void Repair::sendVerifyingFilesNotification() {
    ...
    for (int i = 0; i < this->nzbFileDataList.size(); i++) {
        ...
        if ((par2ByteArray.contains(nzbFileData.getDecodedFileName().toAscii())) ||
            ...
            // notifty user :
            emit updateRepairSignal(nzbFileData.getUniqueIdentifier(), 0, VerifyStatus, this->getItemTarget(nzbFileData));
    ...
    for (int i = 0; i < this->nzbFileDataList.size(); i++) {
        ...
        if (nzbFileData.match(fileNameStr, originalFileNameStr)) {
            ...
            if (fileStatus != VerifyMissingStatus) {
                emit updateRepairSignal(nzbFileData.getUniqueIdentifier(), PROGRESS_COMPLETE, fileStatus, ChildItemTarget);
            }
